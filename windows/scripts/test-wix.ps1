...
# Hack: add %ProgramFiles%\amiga-ci-tools\vasmm68k to path in case it is not already present
...
if (!($Env:Path -like "${Env:ProgramFiles}\amiga-ci-tools\vasmm68k"))
{
    $Env:Path += ";${Env:ProgramFiles}\amiga-ci-tools\vasmm68k"
}
...
if (!(Test-Path "${Env:ProgramFiles}/amiga-ci-tools/vasmm68k/vasm.pdf")) { throw "vasm.pdf is not present in installation folder" }
