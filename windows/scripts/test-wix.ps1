...
# Hack: add %ProgramFiles%\${GITHUB_ORGANIZATION_NAME}\vasmm68k to path in case it is not already present
...
if (!($Env:Path -like "${Env:ProgramFiles}\${GITHUB_ORGANIZATION_NAME}\vasmm68k"))
{
    $Env:Path += ";${Env:ProgramFiles}\${GITHUB_ORGANIZATION_NAME}\vasmm68k"
}
...
if (!(Test-Path "${Env:ProgramFiles}/${GITHUB_ORGANIZATION_NAME}/vasmm68k/vasm.pdf")) { throw "vasm.pdf is not present in installation folder" }
