...
Compress-Archive -ErrorAction Stop -Path "${BUILD_RESULTS_DIR}/temp/vasmm68k_mot.exe", "${BUILD_RESULTS_DIR}/temp/vasmm68k_std.exe", "${BUILD_RESULTS_DIR}/temp/vobjdump.exe", "${BUILD_RESULTS_DIR}/temp/LICENSE.md" -DestinationPath "${BUILD_RESULTS_DIR}/vasmm68k-${VASM_VERSION}-windows-binaries.zip"
Remove-Item -ErrorAction Stop -Force -Recurse "${BUILD_RESULTS_DIR}/temp"
