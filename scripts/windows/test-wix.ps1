...
& "${BUILD_RESULTS_DIR}\\vasmm68k-${VASM_VERSION}-windows-installer.msi" /quiet; if ($LASTEXITCODE -ne 0) { Write-Output "Lastexitcode: ${LASTEXITCODE}"; throw }
refreshenv
...
& msiexec /x "${BUILD_RESULTS_DIR}\\vasmm68k-${VASM_VERSION}-windows-installer.msi" /quiet; if ($LASTEXITCODE -ne 0) { throw }
refreshenv
Remove-Item -ErrorAction Stop -Force -Recurse ${BUILD_RESULTS_DIR}/temp
