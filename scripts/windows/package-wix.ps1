...
& candle.exe wix/vasmm68k.wxs -o "${BUILD_RESULTS_DIR}/temp/vasmm68k.wixobj" -arch x64 "-dApplicationVersion=${VASM_VERSION}"; if ($LASTEXITCODE -ne 0) { throw }
& light.exe "${BUILD_RESULTS_DIR}/temp/vasmm68k.wixobj" -o "${BUILD_RESULTS_DIR}/vasmm68k-${VASM_VERSION}-windows-installer.msi" -loc wix/vasmm68k.wxl; if ($LASTEXITCODE -ne 0) { throw }
Remove-Item -ErrorAction Stop -Force -Recurse "${BUILD_RESULTS_DIR}/temp"
